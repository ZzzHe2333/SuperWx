"""wxauto4 doctor — health check for WeChat 4.x UI controls.

Usage::

    python -m wxauto4 doctor
    python -m wxauto4 doctor --report
    python -m wxauto4 doctor --dump-on-fail
...
def run_doctor(report_path: str = None, dump_on_fail: bool = False) -> int:
    ...
    _log("=" * 60)
    _log("wxauto4 doctor — WeChat 4.x UI health check")
    _log("=" * 60)
    _log()
...
def _auto_dump(root, failed_controls: Dict[str, str]):
    ...
    print("\n--- 自动 dump UI 树 ---")
    try:
        dump_dir = dump_ui_tree(root, output_dir=".wxauto4_repair/dumps")
        print(f"UI 树已保存到: {dump_dir}")
    except Exception as e:
        print(f"dump 失败: {e}")
        dump_dir = ".wxauto4_repair/dumps"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="wxauto4 doctor")
    parser.add_argument("--report", nargs="?", const="wxauto4_doctor_report.txt", help="Save TXT report")
